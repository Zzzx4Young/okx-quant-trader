...
> **版本 v1.7** · 基于 OKX API v5 · 111/111 测试通过 · 4 策略 + 多层风控 + 实时对账 + 回测引擎

[![Python](https://img.shields.io/badge/Python-3.8%2B-blue)](https://www.python.org/)
[![Version](https://img.shields.io/badge/version-1.7.0-orange)](okx/__init__.py)
...
┌─────────────────────────────────────────────────────────────────────┐
│                       OpenClaw 调度层                                │
│  · Cron Job  · Heartbeat · 人工触发（CLI / Web / Telegram）         │
└─────────────────────────────┬───────────────────────────────────────┘
                              │ 调用
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│                    Runner (code/runner.py)                          │
│  ① 前置风控 → ② 持仓管理 (check_and_close) → ③ 信号扫描           │
│  ④ portfolio ↔ OKX 对账 → ⑤ 通知                                    │
└──┬───────────┬──────────┬─────────────┬─────────────┬───────────────┘
   ▼           ▼          ▼             ▼             ▼
┌────────┐ ┌────────┐ ┌────────┐  ┌──────────┐ ┌────────────────┐
...
│K线/ticker│ A/B/C/D │ ATR/RR │  │ OKX v.s.  │ │7 类事件 + dedup│
└────────┘ └────────┘ └────────┘  └──────────┘ └────────────────┘
                                                       │
                                                       ▼
                                              ┌─────────────────┐
...
**控制流**：调度层 → Runner → 5 大模块协作 → 状态写 portfolio.json + portfolio 对账触发 OKX 历史拉取 → TelegramNotifier 推 critical 事件
...
| **Telegram 通知** | 7 类事件（开/平/部分平/错误/daily/heartbeat/drift），5min dedup |
...
| **111 单测** | pytest 全过，含 Constitution 配置验证 + 13 个 A+C double-lock 回归测试 |
...
| 方式 | 适用场景 | 配位置 |
|---|---|---|
| **OpenClaw cron (推荐)** | 个人交易 + 看 watchdog | `~/.openclaw/openclaw.json` 中 cron jobs |
| **Linux crontab** | 纯服务器 + 不要 watchdog | `crontab -e`，示例：`*/5 * * * * cd /path/to/okx && bash run.sh run` |
| **手动** | 调试 / 验证 | `./run.sh run` |
