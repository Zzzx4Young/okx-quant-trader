...
| **回测引擎 v2** | 3 活跃策略 + 1 归档策略 (D) + 2 基准（1x_spot / 5x_leverage 含动态爆仓），完整 metrics + tranche fills |
...
| 文档 | 内容 |
|---|---|
| **[docs/SIGNALS.md](docs/SIGNALS.md)** | 3 活跃策略 + D 归档信号精确定义（参数 + 触发条件） |
...
| 版本 | 日期 | 关键变更 |
|---|---|---|
| **v1.8.3** | **2026-07-19** | **D 策略 `FUNDING_RATE_REVERSAL` 永久禁用** (`config.json: strategy_d.enabled=false`)。fragility_scan 全 timeframe 0 viable (BTC 1h/5m 0 trades, ETH 1h -0.87%, ETH 5m **-15.74%** / Sharpe **-2.669**)。5 根因：funding 极罕见 (BTC ~1次/年) + 8h 结算 vs 5m 错位 + blacklist 矛盾 + RSI 高频噪声 + 不可规模化。软移除 (config gate) 而非硬删除 (代码保留作为未来 D2.0 参考)。 **其他修复**: ⑴ cron signal-runner spinlock fallback (`MAX_WAIT_SECONDS=240`, commit `effb148`); ⑵ portfolio schema validation fail-loud; ⑶ watchdog structural-level 降噪; ⑷ HTTPClient.public fix (market_filter); ⑸ `to_okx_swap_symbol` funding_rate 归一化; ⑹ Portfolio.daily_stats latent bug fix; ⑺ watchdog Layer 1 critical 不再被 format_report 吞掉 (守心不再失灵, `c9d5ef2`)。 7 个 commits + 总测试 404/404。 |
...
| 实测 | N | avg_taker_slip (bps) | p95_taker_slip (bps) | fill_rate (overall) | market_fill_rate | limit_fill_rate | 判定 |
|---|---|---|---|---|---|---|---|
| v7 (2026-07-18) | 50 BTC | 4.19 | 7.99 | 74% | 100% | 48% | ❌ fill_rate < 90% |
| **v8 (2026-07-19)** | **100 BTC** | **1.42** | **4.81** | **65%** | **100%** | **30%** | ❌ fill_rate < 90%；✅ avg_taker_slip **8 bps 红线利用率 18%** |

**v8 关键发现**：(1) N 翻倍后 avg_taker_slip 从 4.19 → 1.42 bps（-66%），比 fragility_scan 默认 5 bps 假设保守 71%；(2) market_fill_rate 100% (API 链路完全 OK)；(3) limit_fill_rate 30-48% (OKX demo 流动性池限制，不是系统问题)。**Taker 核心准入通过，fill_rate 红线需 split 化** (详见 `experiments/diagnose_demo-gate7-btc-n100-v8-20260719-20260719-133248/analysis.md`)。
...
_最后更新：2026-07-19 v1.8.3 (D 永久禁用 + 6 个 P0-P2 修复)_
