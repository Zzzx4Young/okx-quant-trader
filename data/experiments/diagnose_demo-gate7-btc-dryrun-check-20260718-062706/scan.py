        ...
        try:
            if ord_type == "market":
                order = trade.place_order(
                    ...
                    sz=str(size_per_trade), td_mode="cash",
                )
            else:
                order = trade.place_order(
...
def main():
    ...
    parser.add_argument("--limit-offset-bps", type=float, default=5.0, help="限价偏离 bps")
    ...
    client = OKXClient()
    logger.info(f"Mode: {'DEMO' if client.is_demo else '⚠️ LIVE MODE'}")
