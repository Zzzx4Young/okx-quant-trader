...
class TestDeadCodeDocumented:
    ...
    def test_if_whitelist_match_works_must_still_cap_at_hard_ceiling(self, cfg):
        """forward-compatible: 即使 whitelist 匹得上, leverage 必须 ≤ hard_ceiling

        这是真正的合约。如果将来有人"修复" _get_leverage 让 whitelist 匹得上,
        本测试确保他们不会无意中让 BTC/ETH 杠杆升到 5x。
        """
        engine = SignalEngine(market_api=MagicMock(), config=cfg)

        # 模拟 "如果 whitelist 匹配生效" 的场景
        # 直接构造一个高 leverage 值, 验证 _get_leverage 的最终输出 ≤ hard_ceiling
        # 这等价于: 假设 default_leverage_main 被读到, 必须 cap 到 3
        hard_ceiling = cfg.leverage_matrix_btc["hard_ceiling"]
        capped = min(cfg.default_leverage_main, hard_ceiling)

        assert capped == 3, (
            f"min(default={cfg.default_leverage_main}, hard_ceiling={hard_ceiling}) "
            f"必须 = 3, got {capped}"
        )

        # 验证 _get_leverage 实际行为 (不管底层匹配逻辑) 仍然 ≤ hard_ceiling
        actual_lev = engine._get_leverage("BTC-USDT-SWAP")
        assert actual_lev <= hard_ceiling, (
            f"BTC leverage 必须 ≤ hard_ceiling={hard_ceiling}, got {actual_lev}"
        )


# ──────────── Layer 4: 端到端 - 模拟 set_leverage 调用参数 ────────────

class TestEndToEndLeverageContract:
    """
    端到端合约: OKX 收到的 lever 参数必须 ≤ hard_ceiling。

    这是 system boundary test。Mock OKX client, 捕获 set_leverage 调用,
    验证 lever 参数合规。

    注: 不调 Runner._process_signal (依赖太多前置条件),
        直接构造 Signal → 走 _get_leverage → 模拟 set_leverage 调用。
    """

    def test_btc_set_leverage_receives_3x_not_5x(self, cfg):
        """边界测试: BTC 完整路径, set_leverage 收到 lever='3' 不是 '5'"""
        from unittest.mock import MagicMock

        mock_account = MagicMock()
        mock_account.set_leverage = MagicMock(return_value={"code": "0", "data": []})
        ...
        # 模拟 runner.py:660 set_leverage 调用
        mock_account.set_leverage(
        ...
        # 验证调用参数
        mock_account.set_leverage.assert_called_once()
        actual_lever = mock_account.set_leverage.call_args.kwargs["lever"]
        assert actual_lever == "3", (
            f"set_leverage 必须收到 lever='3' (Phase 5.2), got '{actual_lever}'"
        )